...
class MED:
    ...
    def __init__(self):
        ...
        self.target_chars = ["#"] + [char for char in self.target.replace(" ", "_")]
...
class Alignment(MED):
    ...
    def needleman_wunsch(self, match: int, mismatch: int, gap: int) -> DataFrame:
        ...
        score_matrix = DataFrame(data=score_matrix, index=self.source_chars, columns=self.target_chars)
        m, n = len(self.source), len(self.target)
